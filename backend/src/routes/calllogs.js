...
router.get('/', auth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM call_logs ORDER BY date DESC'
    );
...
router.get('/date-range', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM call_logs WHERE date >= $1 AND date <= $2 ORDER BY date ASC',
      [startDate, endDate]
    );
    res.json(result.rows);
  } catch (error) {
...
router.get('/date/:date', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM call_logs WHERE date = $1 ORDER BY created_at ASC',
      [date]
    );
    res.json(result.rows);
  } catch (error) {
...
router.post('/:id/complete-date', auth, async (req, res) => {
  try {
    ...
    if (!currentCompletedDates.includes(date)) {
      ...
      const result = await pool.query(
        'UPDATE call_logs SET completed_dates = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *',
...
router.post('/:id/convert-date', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE call_logs SET converted_dates = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *',
